...
public class OrderService {
    ...
    public void deleteOrder(long id) {
        Optional<Order> orderDelete = this.fetchOrderById(id);
        if (orderDelete.isPresent()) {
            Order order = orderDelete.get();
    ...
    public void updateOrder(Order order) {
        Optional<Order> orderUpdate = this.orderRepository.findById(order.getId());
        if (orderUpdate.isPresent()) {
            Order orderToUpdate = orderUpdate.get();
